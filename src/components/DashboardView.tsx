...
export default function DashboardView({ 
  ...
  const ihsgCandles = useMemo<SampleCandle[]>(() => {
    const openPrice = ihsgPrevClose || 6007.66;
    const closePrice = ihsgPrice || 6254.97;
    const delta = closePrice - openPrice; // e.g., 247.31
  ...
  const [ihsgHistory, setIhsgHistory] = useState<number[]>([5910, 5930, 5950, 5980, 5990, 6007.66, 6100, 6180, 6220, 6254.97]);
    ...
    <div className="space-y-6">
              ...
              <div className="space-y-1">
                ...
                <div className="relative group flex items-baseline gap-3.5 mt-2.5 font-mono flex-wrap cursor-help">
                  ...
                  <div className="flex flex-row flex-wrap items-center gap-2 mt-1 sm:mt-0">
                    ...
                    {/* Simple trend indicator (Bullish, Bearish, or Sideways) next to changes */}
                    <div 
                      className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs font-bold font-sans border shadow-md ${trendBadgeColor}`}
                      title={trendDesc}
                    >
                      <span className="text-xs">{trendIcon}</span>
                      <span className="uppercase tracking-wide text-[10px]">{trendLabel} (5M)</span>
                    </div>
                    {isIHSGValidating && (
      ...
      <div className="glass-card rounded-2xl p-5 border border-slate-800 bg-slate-900/10 space-y-4">
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
          ...
          {/* Real-time Update Pulse Indicators */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
            
            {/* Live blinking/glowing notification banner */}
            <div className="px-4 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl flex items-center space-x-3 text-xs">
              <div className="relative flex h-2.5 w-2.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500"></span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] uppercase tracking-widest text-[#94a3b8] font-bold leading-none">IHSG Live Feed</span>
                <span className={`text-[11px] font-mono font-bold mt-1 transition-all ${showUpdateToast ? "text-emerald-400" : "text-slate-400"}`}>
                  {showUpdateToast ? "✓ IHSG Terupdate!" : `Data termutakhir (T-${secondsSinceUpdate}s)`}
                </span>
              </div>
            </div>
      ...
      {showDetailHub ? (
        ...
        {/* ========================================================================= */}
        {/* 🇮🇩 SECTION: BERITA EKONOMI MAKRO, MIKRO & GLOBAL                          */}
        {/* ========================================================================= */}
        <div className="glass-card p-6 rounded-2xl border border-cyan-900/30 bg-[#020d18]/90 space-y-6 mt-6 relative overflow-hidden">
          ...
          {/* Header Portal */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-cyan-950/40 pb-5">
            <div className="space-y-1.5 max-w-2xl">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse"></span>
                <h3 className="text-xs font-black uppercase tracking-widest text-cyan-300 flex items-center gap-1.5 font-sans">
                  <Newspaper className="w-4.5 h-4.5 text-cyan-400" /> Portal Analisis Ekonomi Makro, Mikro & Global
                </h3>
              </div>
              <h2 className="text-base sm:text-lg font-black text-white tracking-wide">
                Informasi & Sentimen Kebijakan Moneter, Komoditas, serta Konsolidasi Korporasi
              </h2>
              <p className="text-[11.5px] text-slate-400 leading-relaxed">
                Menyajikan evaluasi berkala mengenai perubahan suku bunga BI-Rate, proyeksi FOMC / The Fed, pergerakan nilai tukar Rupiah (USD/IDR), bursa obligasi SBN, pergerakan harga komoditas utama (CPO, Batubara), serta aksi korporasi domestik.
              </p>
            </div>

            {/* Quick Macro Indicators Panel */}
            <div className="grid grid-cols-2 xs:grid-cols-3 gap-2 shrink-0 md:border-l border-cyan-950/50 md:pl-5 pt-3 md:pt-0">
              <div className="bg-[#020b12] p-2.5 rounded-lg border border-cyan-950/60 flex flex-col justify-between">
                <span className="text-[8px] text-slate-500 uppercase font-black font-mono leading-none">BI-RATE</span>
                <span className="text-xs font-black font-mono text-cyan-400 mt-1">6.25%</span>
                <span className="text-[7.5px] text-slate-500 font-bold uppercase block mt-0.5">TETAP</span>
              </div>
              <div className="bg-[#020b12] p-2.5 rounded-lg border border-cyan-950/60 flex flex-col justify-between">
                <span className="text-[8px] text-slate-500 uppercase font-black font-mono leading-none">FED FUNDS</span>
                <span className="text-xs font-black font-mono text-amber-500 mt-1">5.25%-5.50%</span>
                <span className="text-[7.5px] text-slate-500 font-bold uppercase block mt-0.5">KAWAL INFLASI</span>
              </div>
              <div className="bg-[#020b12] p-2.5 rounded-lg border border-cyan-950/60 flex flex-col justify-between">
                <span className="text-[8px] text-slate-500 uppercase font-black font-mono leading-none">RUPIAH (USD)</span>
                <span className="text-xs font-black font-mono text-emerald-400 mt-1">Rp 16.245</span>
                <span className="text-[7.5px] text-emerald-500 font-bold uppercase block mt-0.5">MENGUAT (1d)</span>
              </div>
            </div>
          </div>

          {/* Filtering Sub-Tabs */}
          <div className="flex flex-wrap gap-1.5 bg-slate-950/50 p-1 rounded-xl border border-slate-900/40 text-[10.5px] md:text-xs">
            {(["SEMUA", "MAKRO INDONESIA", "GLOBAL FEED", "KOMODITAS GLOBAL", "MIKRO KORPORASI", "BLOOMBERG TECHNOZ"] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => setNewsFilterTab(tab)}
                className={`py-1.5 px-3 md:px-4 rounded-lg font-black tracking-wider transition-all cursor-pointer ${
                  newsFilterTab === tab 
                    ? "bg-[#0b293c] text-cyan-300 border border-cyan-500/20 shadow-sm" 
                    : "text-slate-400 hover:text-white"
                }`}
              >
                {tab}
            ...
            {(() => {
              const economyNews = [
                {
                  ...
                  title: "Rapat Dewan Gubernur BI Pertahankan BI-Rate di Level 6,25% Demi Resiliensi Rupiah",
                  description: "Bank Indonesia memutuskan untuk memantapkan suku bunga acuan (BI-Rate) pada level 6,25% guna menjaga sasaran inflasi nasional di rentang target 2,5±1% dan memastikan capital flow tetap sehat menyikapi geopolitik global.",
                  source: "Bloomberg Technoz",
                  time: "10 Menit Lalu",
                  sentiment: "neutral",
                  impact: "Sektor perbankan (BBRI, BMRI, BBNI) diperkirakan stabil dengan net-interest margin (NIM) terjaga."
                },
                {
                  id: "global_1",
                  category: "GLOBAL FEED",
                  title: "Proyeksi FOMC Rate Bergeser, Spekulasi Pemotongan Suku Bunga AS Mundur ke Sesi Akhir Tahun",
                  description: "Data tenaga kerja Amerika Serikat yang tak terduga ketat meredam ekspektasi pemotongan suku bunga Fed Funds Rate secara agresif dalam jangka pendek. Indeks Dollar (DXY) bertahan kokoh di kisaran 104,80.",
                  source: "CNBC International",
                  time: "45 Menit Lalu",
                  sentiment: "bearish",
                  impact: "Aliran dana asing (capital flow) di aset investasi berisiko tinggi bursa domestik cenderung konsolidatif."
                },
                {
                  id: "komoditas_1",
                  category: "KOMODITAS GLOBAL",
                  title: "Harga Crude Palm Oil (CPO) Terkerek Naik Menuju RM 4.150 per Ton Terpapar Kebijakan B40",
                  description: "Harga komoditas CPO di bursa Malaysia menguat signifikan didorong oleh akselerasi pengujian program wajib bauran solar B40 oleh Kementerian ESDM yang diperkirakan mengunci konsumsi lokal berskala besar.",
                  source: "Kontan Indonesia",
                  time: "2 Jam Lalu",
                  sentiment: "bullish",
                  impact: "Sinyal positif kuat untuk produsen kelapa sawit nasional seperti AALI, LSIP, dan DSNG."
                },
                {
                  id: "mikro_1",
                  category: "MIKRO KORPORASI",
                  title: "Aksi Sinergi Frekuensi Telekomunikasi EXCL & ISAT Targetkan Penghematan Belanja Modal 15%",
                  description: "Manajemen EXCL dan ISAT terus kaji kerja sama pemakaian frekuensi nirkabel bersama serta menara pintar 5G demi menekan pengeluaran belanja modal jumbo sektor telekomunikasi Indonesia pada sisa tahun anggaran berjalan.",
                  source: "Bisnis Indonesia",
                  time: "3 Jam Lalu",
                  sentiment: "bullish",
                  impact: "Mempercepat pencapaian margin EBITDA dan free cash flow sektor telekomunikasi."
                },
                {
                  id: "macro_2",
                  category: "MAKRO INDONESIA",
                  title: "Surplus Neraca Perdagangan Indonesia Berlanjut 48 Bulan Beruntun Dipacu Nikel & Besi Baja",
                  description: "BPS mengumumkan rilis neraca perdagangan April mencatat surplus US$ 3.56 Miliar. Sektor hilirisasi nikel dan manufaktur smelter logam jadi menjadi penyumbang terbesar ekspor non-migas Indonesia.",
                  source: "IDX Channel",
                  time: "5 Jam Lalu",
                  sentiment: "bullish",
                  impact: "Penguat fundamental rupiah, cadangan devisa terpelihara kokoh di kisaran US$ 140 Miliar."
                },
                {
                  id: "komoditas_2",
                  category: "KOMODITAS GLOBAL",
                  title: "Sanksi Baru Hambat Impor Gas Eropa, Harga Batu Bara Thermal Kembali Ke Area US$ 141 / Ton",
                  description: "Terkendala hambatan suplai gas pipa dan cuaca ekstrim di belahan Asia Timur, permintaan batu bara berkalori tinggi dari bursa Newcastle naik tajam sebesar 3,8% dalam rentang sepekan kemarin.",
                  source: "Reuters Financial",
                  time: "7 Jam Lalu",
                  sentiment: "bullish",
                  impact: "Katalis pendorong margin ekspor komoditas andalan Indonesia (ADRO, PTBA, ITMG)."
                },
                {
                  id: "mikro_2",
                  category: "MIKRO KORPORASI",
                  title: "Konglomerat Infrastruktur Energetik Siapkan Emisi Green Bond Rp 4.5 Triliun dengan Rating AA+",
                  description: "Pefindo memberikan peringkat AA+ bagi rencana penerbitan Green Bonds jangka panjang guna refinancing utang lama dan pembiayaan proyek Pembangkit Listrik Tenaga Surya (PLTS) Terapung berkapasitas tinggi.",
                  source: "Dunia Investasi",
                  time: "10 Jam Lalu",
                  sentiment: "neutral",
                  impact: "Mengurangi biaya modal pinjaman korporasi dan menciptakan alternatif investasi ramah lingkungan."
                },
                {
                  id: "global_2",
                  category: "GLOBAL FEED",
                  title: "Laju Inflasi Zona Euro Melambat ke Level 2.4% Membuka Ruang ECB Pangkas Bunga Eropa Terlebih Dulu",
                  description: "Melambatnya laju pertumbuhan indeks harga barang di Eropa mendorong bank sentral ECB dinilai memiliki amunisi tinggi untuk melonggarkan kebijakan moneter mendahului The Fed dlm periode waktu dekat.",
                  source: "Bloomberg",
                  time: "1 Hari Lalu",
                  sentiment: "bullish",
                  impact: "Dapat memicu perpindahan arus likuiditas investor global ke instrumen obligasi negara bermargin tinggi (SBN RI)."
                },
                {
                  id: "bt_1",
                  category: "MAKRO INDONESIA",
                  title: "Arus Kas Masuk Sektor Properti Domestik Melonjak Drastis Dipicu Insentif Pajak PPN DTP",
                  description: "Bloomberg Technoz melaporkan peningkatan gairah investasi di emiten developer residensial menyusul perpanjangan kebijakan Pajak Pertambahan Nilai Ditanggung Pemerintah secara penuh hingga akhir kuartal bursa tahun ini.",
                  source: "Bloomberg Technoz",
                  time: "1 Hari Lalu",
                  sentiment: "bullish",
                  impact: "Sentimen sangat positif untuk BSDE, CTRA, PWON, dan SMRA."
                },
                {
                  id: "bt_2",
                  category: "KOMODITAS GLOBAL",
                  title: "Smelter Baru Siap Beroperasi Penuh, Hilirisasi Nikel Dorong Penguatan Neraca Dagang Domestik",
                  description: "Dengan masuknya investasi Smelter berteknologi HPAL terbaru di Indonesia Timur, Bloomberg Technoz memproyeksikan peningkatan output nikel baterai kelas satu yang siap menyuplai rantai pasok global.",
                  source: "Bloomberg Technoz",
                  time: "2 Hari Lalu",
                  sentiment: "bullish",
                  impact: "Menjadi sentimen pendorong utama kinerja jangka menengah bagi INCO, ANTM, dan MDKA."
                }
              ];

              const filteredNews = economyNews.filter(
                (n) => {
                  if (newsFilterTab === "SEMUA") return true;
                  if (newsFilterTab === "BLOOMBERG TECHNOZ") return n.source === "Bloomberg Technoz";
                  return n.category === newsFilterTab;
                }
              );
              ...
              return filteredNews.map((news) => {
                ...
                return (
                  <div 
                    key={news.id}
                    className="p-4 bg-[#010910] hover:bg-[#021321] border border-cyan-950/60 rounded-xl space-y-3 transition-all duration-200 flex flex-col justify-between group"
                  >
                    <div className="space-y-2">
                      {/* Top labels */}
                      <div className="flex items-center justify-between">
                        <span className="text-[9px] font-black uppercase text-cyan-400 bg-cyan-950/50 border border-cyan-900/30 px-2 py-0.5 rounded font-mono">
                          {news.category}
                        </span>
                      ...
                      {/* Title */}
                      <h4 className="text-[12.5px] font-black text-slate-100 group-hover:text-white transition-colors tracking-tight leading-snug">
                        {news.title}
                      </h4>

                      {/* Description */}
                      <p className="text-[11px] text-slate-400 leading-normal font-sans text-justify">
                        {news.description}
                      </p>
                    </div>

                    {/* Impact Box */}
                    <div className="pt-2.5 border-t border-cyan-950/40 mt-1.5 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[9px] text-slate-500 font-mono uppercase font-black">SENTIMEN DAMPAK:</span>
                        <span className={`px-1.5 py-0.5 text-[8.5px] font-black rounded-sm font-mono ${
                          isBullish 
                            ? "bg-emerald-950/50 text-emerald-400 border border-emerald-900/20" 
                            : isBearish 
                              ? "bg-rose-950/50 text-rose-450 border border-rose-900/20" 
                              : "bg-slate-900 text-slate-450 border border-slate-800"
                        }`}>
                          {isBullish ? "🟢 POSITIF (BULLISH)" : isBearish ? "🔴 NEGATIF (BEARISH)" : "⚪ NETRAL"}
                        </span>
                      </div>
                      <p className="text-[10px] text-cyan-300/80 bg-cyan-950/10 p-2 rounded border border-cyan-950/35 font-sans leading-relaxed text-justify">
                        <strong className="text-[8.5px] uppercase font-mono text-cyan-500 font-black block">Implikasi Pasar:</strong>
                        {news.impact}
                      </p>
                    </div>
                  </div>
          ...
          {/* Disclaimer Footer directly from reference image */}
          <div className="p-3 bg-cyan-950/10 border border-cyan-900/20 rounded-xl flex items-center gap-3 text-[10px] text-slate-400 leading-relaxed font-sans mt-2">
            <span className="p-1 rounded bg-[#01111d] text-cyan-400 font-bold font-mono shrink-0">DYOR INDEK</span>
            <p>Data diatas merupakan agregasi feed intelijen ekonomi makro-mikro dari bursa dan lembaga pers terpercaya. Harap selalu lakukan verifikasi tambahan (Double analysis) sebelum melakukan penempatan dana riil di pasar saham reguler IDX Indonesia.</p>
          </div>
